...
class CloudformationTests(unittest.TestCase):
    ...
    @patch('boto3.client')
    def test_get_stacknames_by_status(self, mock):
        client = MagicMock()
        client.list_stacks.side_effect = self.cf_list_stacks_mock
        mock.return_value = client

        result = cloudformation.get_stacknames_by_status(status='DELETE_FAILED')
        self.assertEqual(1, len(result))
        self.assertEqual('stack-DELETE_FAILED', result[0])

    @patch('click.confirm')
    @patch('aws_account_janitor.cloudformation.get_stacknames_by_status')
    @patch('boto3.client')
    def test_cleanup__happy_path(self, mock_boto3, mock_get_stacknames_by_status, mock_click_confirm):
        client = MagicMock()
        mock_boto3.return_value = client
        mock_get_stacknames_by_status.return_value = ['foo-stack', 'bar-stack']
        mock_click_confirm.return_value = True

        cloudformation.cleanup(status='DELETE_FAILED')
        mock_get_stacknames_by_status.assert_called_with('DELETE_FAILED')
    ...
    @patch('aws_account_janitor.cloudformation.get_stacknames_by_status')
    @patch('boto3.client')
    def test_cleanup__not_confirmed(self, mock_boto3, mock_get_stacknames_by_status, mock_click_confirm):
        client = MagicMock()
        mock_boto3.return_value = client
        mock_get_stacknames_by_status.return_value = ['foo-stack']
        mock_click_confirm.return_value = False

        cloudformation.cleanup(status='FOO')
        mock_get_stacknames_by_status.assert_called_with('FOO')
        client.delete_stack.assert_not_called()

    @patch('aws_account_janitor.cloudformation.get_stacknames_by_status')
    @patch('boto3.client')
    def test_cleanup__non_interactive(self, mock_boto3, mock_get_stacknames_by_status):
        client = MagicMock()
        mock_boto3.return_value = client
        mock_get_stacknames_by_status.return_value = ['foo-stack']
